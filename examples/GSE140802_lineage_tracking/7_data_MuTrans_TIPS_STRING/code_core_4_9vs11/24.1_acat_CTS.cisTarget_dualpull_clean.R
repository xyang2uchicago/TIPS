...
tips_extend_r <- tips_source_extend(
  wd = get0("wd", ifnotfound = NULL),
  code_dir = code_dir
)
message("[24.1] TIPS_extend: ", tips_extend_r)
...
tf_symbol_from_ppi_filename <- function(f) {
  m <- regexec("^PPI_graph_([^_]+)_GRN_prediction_.*_final\\.rds$", f, perl = TRUE)
  hit <- regmatches(f, m)[[1L]]
  if (length(hit) < 2L) return(character())
  hit[[2L]]
}

## Felix code_core_13 / Ibarra endothelial.b: cisTarget row for dualpull_final_table motif columns
cistarget_motif_annotation_row <- function(
    tf_key, motifAnnot_sub, cisTarget.res, CTS_ID, NES_threshold
) {
  tf_key <- as.character(tf_key)
  reg <- as.character(motifAnnot_sub$regulators)
  y <- motifAnnot_sub[
    reg %in% tf_key | grepl(tf_key, reg, ignore.case = TRUE),
    ,
    drop = FALSE
  ]
  motif_vals <- unique(y$motif_TF_highConf)
  motif_vals <- motif_vals[!is.na(motif_vals) & nzchar(motif_vals)]
  if (!length(motif_vals)) {
    return(data.frame(TF_highConf = NA, motif = NA, NES = NA))
  }
  gs_ok <- as.character(cisTarget.res$geneSet) == as.character(CTS_ID)
  tmp <- cisTarget.res[
    gs_ok &
      !is.na(cisTarget.res$NES) &
      cisTarget.res$NES >= NES_threshold &
      cisTarget.res$motif %in% motif_vals,
    ,
    drop = FALSE
  ]
  if (!nrow(tmp)) {
  ...
  tmp[1L, c("TF_highConf", "motif", "NES"), drop = FALSE]
}

tf_key_from_ppi_filename <- function(f, key_TFs) {
  pattern <- paste(key_TFs, collapse = "|")
  key <- regmatches(f, regexpr(pattern, f, perl = TRUE))
  if (length(key) && nzchar(key)) return(key)
  tf_symbol_from_ppi_filename(f)
}
...
for (f in files) {
  ...
  tmp1 <- cistarget_motif_annotation_row(
    key, motifAnnot_sub, cisTarget.res, CTS_ID, NES_threshold
  )
  change_df <- cbind(
