...
assert_sce_orientation <- function(sce) {
  ...
  ## [Caesar] cell-level input has cells > genes by construction (3000 x 92527).
  ## Gated so metacell behaviour is bit-for-bit unchanged by default.
  if (nc > nr && !identical(Sys.getenv("TIPS_ALLOW_CELL_LEVEL"), "TRUE")) {
    stop(
...
assert_sce_gene_overlap <- function(
  ...
  ## [Caesar] min_overlap is an absolute count tuned to metacell-sized HiG sets.
  ## Cell-level HiG networks are smaller (18-107 vertices) though overlap is 100%.
  if (ov < min_overlap && !identical(Sys.getenv("TIPS_ALLOW_CELL_LEVEL"), "TRUE")) {
    stop(
