...
tips_configure <- function(
    ...
    wd = Sys.getenv(
      "TIPS_WD",
      here::here("examples", "hematopoietic_LARRY", "metacell", "7_data_MuTrans_TIPS_STRING/")
    ),
    envir = .GlobalEnv
) {
...
load_larry_sce <- function() {
  .code_dir <- if (exists("code_dir", inherits = TRUE)) {
    ...
    file.path(here::here("examples", "hematopoietic_LARRY", "metacell", "7_data_MuTrans_TIPS_STRING"), "code_core_4_9vs11")
  }
