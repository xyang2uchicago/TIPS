## 24.0_acat_load_input_clean.R — Felix code_core_13 (Larry MuTrans, mm10 cisTarget)
##
...
## Larry MuTrans: native symbols + mm10 + motifAnnotations_mgi (below)
...
if (file.exists(coding_genes_mouse_rds)) {
  coding_genes <- unique(readRDS(coding_genes_mouse_rds))
} else {
...
if (!file.exists("cisTarget_targets_in_all_CTS.rds")) {
  ...
  dbFile <- cistarget_mm10_db
  feather_path <- dirname(cistarget_mm10_feather)
  if (!dir.exists(feather_path)) dir.create(feather_path, recursive = TRUE)
  if (!file.exists(cistarget_mm10_feather)) {
    options(timeout = 600)
    download.file(cistarget_mm10_url, destfile = cistarget_mm10_feather, mode = "wb", method = "libcurl")
  }
  motifAnnot <- load_motif_annotations_mgi()
  motifRankings <- importRankings(cistarget_mm10_feather)
...
if (identical(species, "mouse")) {
  ...
  message("[24.0] uppercase mouse symbols done (Felix code_core_13)")
}
