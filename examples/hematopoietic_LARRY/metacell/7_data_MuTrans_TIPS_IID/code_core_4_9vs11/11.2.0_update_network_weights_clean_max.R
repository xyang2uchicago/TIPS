...
if (paste0("HiG_", CM_cluster) %in% names(graph_list)) {
  hig_cm <- paste0("HiG_", CM_cluster)
  assert_sce_gene_overlap(
    sce, V(graph_list[[hig_cm]])$name,
    min_overlap = 100L, label = paste0(hig_cm, " graph vertices")
  )
}
...
if (step2) {
  ...
  for (s in specificity_methods) {
    ...
    weighted_out <- paste0(db, "_", ppi_tag, "_graph_perState_simplified_", s, "weighted.rds")
    saveRDS(weighted_graph_list, file = weighted_out)
    message("[11.2.0] wrote ", weighted_out)
  }
}

message("[11.2.0] next: source('11.3_CTS_network_ANND_pagerank.R'), then 12.0")
message("After 12.0 — PageRank: res_pr[[paste0('HiGCTS_', CP_cluster)]]")
message("After 12.0 — BC:       res_bw[[paste0('HiGCTS_', CP_cluster)]]")