...
tips_configure <- function(
    ...
    wd = Sys.getenv(
      "TIPS_WD",
      here::here("examples", "hematopoietic_LARRY", "metacell", "7_data_MuTrans_TIPS_IID/")
    ),
    envir = .GlobalEnv
) {
  ...
  iid_global_rds <- normalizePath(
    Sys.getenv(
      "TIPS_IID_GLOBAL_RDS",
      here::here("examples", "Shared_Data", "iid2025_human_mouse_conserved_global.rds")
    ),
    winslash = "/", mustWork = FALSE
  )
...
load_larry_sce <- function() {
  .code_dir <- if (exists("code_dir", inherits = TRUE)) {
    ...
    file.path(here::here("examples", "hematopoietic_LARRY", "metacell", "7_data_MuTrans_TIPS_IID"), "code_core_4_9vs11")
  }
