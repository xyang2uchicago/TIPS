# source('F:/projects/scRNA/source/cardiac_CTS_GRN/GSE175634_iPSC_CM_weighted_v9/24.4.1_overlap_with_publications.R')
...
names(GS)
# [1] "PCGC_AllCurated"                  "PCGC_Valdiated"                  
 # [3] "Takeuchi_gp5"                     "Takeuchi_gp1"                    
 # [5] "Takeuchi_gp10"                    "Takeuchi_gp28"                   
 # [7] "Takeuchi_gp2"                     "Xie_Common"                      
 # [9] "Xie_SHF specific"                 "Xie_JCF specific"                
# [11] "Elorbany_TD_A10_to_9"             "Elorbany_TD_A10_to_4"            
# [13] "Elorbany_Regulon3"                "Elorbany_Regulon1"               
# [15] "Elorbany_Regulon5"                "Elorbany_C3_RegulonPOU5F1_target"
# [17] "Elorbany_C1_RegulonMYRF_target"   "Elorbany_C1_RegulonNFIB_target"  
# [19] "Elorbany_C1_RegulonPLAGL1_target" "Elorbany_C1_RegulonSOX4_target"  
# [21] "Elorbany_C1_RegulonWT1_target"    "Elorbany_C5_RegulonMEF2A_target" 
# [23] "Elorbany_C5_RegulonSMAD9_target"  "Elorbany_C5_RegulonSRF_target"   
# [25] "Maven2023_gene_ISL1_targets_CP_E" "Maven2023_gene_ISL1_targets_CP_T"
# [27] "Maven2023_gene_ISL1_targets_CP_L" "Maven2023_gene_ISL1_WT_d6CP"     
# [29] "Maven2023_gene_ISL_up_E"          "Maven2023_gene_ISL_up_T"         
# [31] "Maven2023_gene_ISL_up_L"          "Maven2023_gene_ISL_dn_E"         
# [33] "Maven2023_gene_ISL_dn_T"          "Maven2023_gene_ISL_dn_L"    
  
length(union(rownames(sce), unique(unlist(df)))) # 25852
universe = union(rownames(sce), unique(unlist(df)))
length(universe) # 25852
universe = union(unlist(GS), universe)
length(universe) # 26514


# CHD_gp = grep('CHD_', names(GS), value=T) %>% gsub('CHD_gp', '', . )
# CHD_gp
 # # [1] "5"   "1"   "10"  "28"  "2"   "96"  "13"  "32"  "9"   "31"  "7"   "29"  "16"  "12"  "6"   "45"  "241"
...
tmp <- enrich_res %>%
  ...
  #remove odds_ratio = Inf which is self comparison
  dplyr::filter(odds_ratio != Inf) %>%
...
# excluding up/downregulated genes and Regulon target genes
tmp2 = subset(tmp, !(grepl('up', tmp$GS) | grepl('dn', tmp$GS) | grepl('Regulon', tmp$scRNA_signature))) 
dim(tmp2) # 19 11

# regulon targets vs other GSs
tmp3 = subset(tmp,  !(grepl('up', tmp$GS) | grepl('dn', tmp$GS) | grepl('Regulon', tmp$GS)) & grepl('Regulon', tmp$scRNA_signature))
dim(tmp3) # 48 11

pdf(file='GS_CTS_or_DEG_padj0.05_top15.pdf', height=10, width=10)  #!!!!!!!!!!!!!!
levels = c('CTS_CP', 'CTS_CP.1',"Elorbany_TD_A10_to_9", "Elorbany_TD_A10_to_4", 'HiG_1', 'HiG_5', 'HiG_3')
p = my_dot_plot(tmp2, ntop=15, ncol=2, CTS_name=c('CP','CP.1'), CP_cluster=CP_cluster, CM_cluster=CM_cluster, Levels=levels)  # excluding up/downregulated genes and Regulon target genes
print(p)
dev.off()

### replot CTS  specifically 
tmp2_CTS = subset(tmp2, scRNA_signature %in% c('CTS_CP.1'))
dim(tmp2_CTS) # 25   11
pdf(file='GS_CTS.CP.1_padj0.05_top15_ordered_by_GS_source.pdf', height=7, width=10)  #!!!!!!!!!!!!
...
tmp2_CTS = subset(tmp2, scRNA_signature %in% c('CTS_CP'))
dim(tmp2_CTS) # 25   11
pdf(file='GS_CTS.CP_padj0.05_top15_ordered_by_GS_source.pdf', height=7, width=10)  #!!!!!!!!!!!!
p = my_dot_plot2(tmp2_CTS, ntop=15, ncol=2, CTS_name='CP', CP_cluster=CP_cluster, CM_cluster=CM_cluster)  # excluding up/downregulated genes and Regulon target genes
print(p)
dev.off()



pdf(file='GS_regulon_vs_other_padj0.05_top15.pdf', height=7.5, width=8)
p = my_dot_plot(tmp3, ntop=15, ncol=2, CTS_name=c('CP','CP.1'), CP_cluster=CP_cluster, CM_cluster=CM_cluster)  # Regulon target genes vs other GSs
print(p)
dev.off()
...
unique(tmp$GS)
int = c( "Elorbany_C3_RegulonPOU5F1_target", "Elorbany_C1_RegulonWT1_target" ,  "Elorbany_C5_RegulonMEF2A_target" ,
  "Takeuchi_gp5"     ,  "Takeuchi_gp28"     ,    "Takeuchi_gp1"     )
unique(tmp$scRNA_signature)

  
tmp = subset(tmp, GS %in% int & scRNA_signature %in% c("CTS_CP" , "CTS_CP.1", "HiG_3"  , "HiG_1"  , "HiG_5" ))
dim(tmp) # 19 11
tmp$scRNA_signature = factor(tmp$scRNA_signature, levels =  c("CTS_CP" , "CTS_CP.1", "HiG_3"  , "HiG_1"  , "HiG_5" ))
