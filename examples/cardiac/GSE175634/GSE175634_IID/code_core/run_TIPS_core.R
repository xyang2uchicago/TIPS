...
##
## CP and CP.1 are not interchangeable via a parameter loop
...
## (no HiGCTS_CP.1 seed TFs identified by 12.0). This branch stops at 24.1's
## stop("No key TFs found...") at NES=4.5 (EBF1/BCL6B are enriched but not in
## DEG of any cluster); tryCatch lets that halt skip past cleanly instead of
## aborting the driver after CP's results are already saved.
setwd(results_dir)
tryCatch({
