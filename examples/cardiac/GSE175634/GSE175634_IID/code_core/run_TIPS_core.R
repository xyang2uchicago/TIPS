## cardiac GSE175634 (human iPSC-CM differentiation) — IID arm
## Full docs: README.md in this folder
## USER PARAMETERS below → tips_configure() → 11.1 … 12.0 → {24.1, 24.3} for CP, then {24.1, 24.3}_CP.1
##
...
## override, a different identify_TF_targeted_pull_candidate fallback for the
## missing HiG_CP.1 graph) — verified by diffing against the CP files. Each
## pair is dispatched to its own file rather than collapsed into a loop.
...
tips_configure(
  ...
  NES_threshold = NES_threshold, core_count = core_count
)
...
## 24.1's stop("No key TFs found...") at NES=4.5 (EBF1/BCL6B enriched but not
## in DEG of any cluster) — tryCatch lets that halt skip past cleanly instead
## of aborting the driver after CP's results are already saved.
setwd(results_dir)
tryCatch({
