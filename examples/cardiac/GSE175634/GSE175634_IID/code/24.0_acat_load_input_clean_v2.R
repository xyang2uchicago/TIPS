...
## the parameter to be assigned before runnign to discuss reproducibility !!!
# $CF_cluster = '1'  # mesenchymal/ECM‑high/CF (“CF‑like”).
# create fold if not existing	
fpath = paste0('F:/projects/scRNA/results/cardiac_CTS_GRN/GSE175634_iPSC_CM_weighted_IID/GSE181346_heart_scATAC_C3vs',CF_cluster)
...
db_specifc_CTS_path = 'F:/projects/TIPS/data/GSE175634_iPSC_CM_weighted_IID/'
...
coding_genes = readRDS( file= paste0(shared_input_path, 'coding_genes.rds'))  %>% unique
...
CHD = readRDS( paste0(shared_input_path, 'CHD_Cilia_Genelist.rds'))
names(CHD)
...
lengths(Ameen_annotation_list)
#  iPSC  iPSC-Mes     i-Mes      i-CP    i-MyoF i-Mes-End     i-pCM      i-CM      i-EC     i-EPC 
      # 769       662       677       468        95       425       371       511       549       205 
    # i-SMC      i-CF       aCM       aEC       Cap        CF       CFP       eCM     Endo1     Endo2 
      # 179       351       262       221       217       141       206       263       399        22 
      # EPC       FB1       FB2       lEC        NC       OFT        PC     preCF    preSMC       vCM 
       # 81       324        59        83       143       262        52         2        75       476 
      # vEC       SMC 
      # 176        72 
...
load(file=paste0(db_specifc_input_path, 'CTS_Lib_Scaledata.RData'))
...
# load(paste0(db_specifc_input_path, "DEG.wc_padj0.01_score40.RData"), envir = new_object) 
...
load(paste0(db_specifc_input_path,'DEG.wc_padj0.01_score40.RData'))
...
########################################################
##  input 4 -- data-deriven --- IID v12 PPI 
## get the whole IID v12 genes
library(igraph)
# refer to 11.2.0_weighted_graph_attack_robustness.R
file = 'F:/projects/scRNA/results/cardiac_CTS_GRN/GSE175634_iPSC_CM_weighted_IID/GSE175634_IID_graph_perState_simplified_combinedweighted.rds'
...
if(species == 'mouse') {
    for(i in 1:length(graph_list)) V(graph_list[[i]])$name = toupper(V(graph_list[[i]])$name)
    for(i in 1:length(DEG)) DEG[[i]] = toupper(DEG[[i]])
    for(i in 1:length(CTS)) CTS[[i]] = toupper(CTS[[i]])
    rownames(sce) = toupper(rownames(sce))
    cat('upcase the mouse symbols to match the gene in the annotation datasets (e.g. TF_human, pseudo scATAC-seq accessibility data) done\n')
}
graph_list[[1]]
DEG[[1]]
CTS[[1]]
...
for(CTS_name in c('CP', 'CP.1')) {
	...
	#if(CTS_name == 'CP.1') seed_TF = c("ISL1" , "HAND2", 'ETS2') else seed_TF = c('HOBX') ## ETS2 was not identified but always cobinding with ISL1 and HAND2	
    ...
    ## here, we annotation the accessibility according to the publisehed main figure 1
...
# FOr E8.25 snapshot, cell are to early to show CF accessibility, thus the accessibility of PCW6 CF clusters are included for ‘CF_accessibility’ (FB1, preCF) 
	...
	# [1] "IGFBPL1"  "OSR1"     "FAM213A"  "TSPAN7"   "CLU"      "GM266"    "IRX1"     "NR2F2"    "PDCD4"   
	# [10] "VEGFC"    "TRABD2B"  "OLFML3"   "SPRY1"    "ELMO1"    "EPHA7"    "PLXNA2"   "CHD3"     "RHOU"    
	# [19] "NRG1"     "RARB"     "EPHA2"    "B3GALNT1" "CDK6"     "FAM110B"  "DACH1"    "NCALD"    "BATF"    
	# [28] "SH3KBP1"  "ROBO2"    "STOX2"  
...
if(!file.exists(file= "cisTarget_targets_in_all_CTS.rds"))  {
	...
	feather_path = 'F:/projects/Share/TF/cistarget/homo_sapiens/hg38/refseq_r80/mc_v10_clust/gene_based/'
	...
	# --- 2) calcualte the enriched motifs amogn CTS genes 
