...
# ATTENTION: MANUAL INPUT REQUIRED AFTER LINE ~150 (search "MANUAL INPUT REQUIRED"
# below). This script performs a live BioMart lookup against the CURRENT
# Ensembl release (not version-pinned, unlike STRINGdb which is pinned to
# "12.0") and then applies HARDCODED ROW-INDEX removals that were manually
# determined by inspecting that lookup's output for this specific dataset.
# It is safe to re-run end-to-end (the duplicate list is regenerated fresh
# from this same run's graph_list), but if BioMart/STRING results ever drift
# — e.g. Ensembl updates an annotation, or a new duplicate gene symbol
# appears — the hardcoded indices below will silently target the wrong rows
# instead of erroring. Re-verify the `annotated_dup` printout for each
# affected cluster by hand before trusting the fix if that happens.
...
markers.up <- readRDS(paste0(data_dir, "DEG_perState_min.prop0.25_lfc0.6_FDFR0.01.rds"))
