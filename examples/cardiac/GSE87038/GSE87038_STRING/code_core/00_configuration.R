## 00_configuration.R — cardiac GSE87038 (E8.25 mouse gastrulation), STRING arm
##
...
tips_configure <- function(
    ...
    CP_cluster    = "8",
    CM_cluster    = "17",
    CF_cluster    = "16",   # STRING uses CF=16, not CF=18 (differs from IID)
    CTS_ID        = CP_cluster,
    seed_TF       = "ISL1",   # from HiGCTS_8 top PageRank (code 12.0); overwritten when 12.0 runs
    NES_threshold = 3,
    logFC.cut     = 0.6,
    ...
    download_files  = FALSE, # set TRUE once to fetch STRING PPIN files + BioTIP.res.RData
    wd = here::here("examples", "cardiac", "GSE87038", "GSE87038_STRING/"),
    envir = .GlobalEnv
) {
  ...
  data_dir    <- paste0(wd, "../data/")   # GSE87038/data/, shared with the IID arm
  ...
  message(
    "[tips_configure] GSE87038_STRING species=", species,
