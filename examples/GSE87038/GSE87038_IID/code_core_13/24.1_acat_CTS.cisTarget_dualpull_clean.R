...
rebuild_mat <- TRUE
source("/Users/felixyu/Documents/GitHub/TIPS/examples/GSE87038/GSE87038_IID/code_core_13/24.0_acat_load_input_clean.R")
...
names(graph_list)
# [1] "HiG_1"     "HiG_2"     ...  "HiG_13"  ...  "CTS_13"  "CTS_8"
...
if (length(files) > 0) {
    ...
    # (same as STRING code_core_13: CTS_13 gene list is identical — same BioTIP output)
    ...
    #              PLAG1, FOXD2, FOXA2 (from respective motifs)
...
## exclusively highly expressed (HiGCTS) in CM (or CF)
library(BioNet)
...
for (f in files) {
    ...
    if (is.null(g2)) {
        cat("Skipping", f, "— g_descendant_sub is NULL\n")
        next
    }
...
dim(final_table) # [1] 26 13  (CF: 23 rows across FOXA2/GATA3/HAND2/SOX17 subnetworks; CM: 3 rows)
...
#     CDH5→PECAM1(unchanged)
# CF HAND2 (NES=5.17): ETS1→HAND2(-), HAND2→TIE1(+), HAND2→PLXND1(+), HAND2→KCNE3(+),
#     HAND2→TSPAN8(+), HAND2→RHOJ(+), CD38→HAND2(+), HAND2→UBASH3B(+)
# CF SOX17 (NES=3.63): SOX17→TIE1(+), MYZAP→SOX17(-), DUSP2→SOX17(+)
