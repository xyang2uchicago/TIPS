...
dir.create(file.path(updir, paste0("ChIPseq_predicted_", CTS_name)),
           showWarnings = FALSE, recursive = TRUE)
setwd(paste0(updir, '/ChIPseq_predicted_',CTS_name))
...
# Step 1.3) heatmap confirming key TF’s self impact by checking its targets among the CTS_CP, candidates to be the highest pagerank TFs !!!!!!!!!!!!!1	
  ...
  #   candidate genes:  16 
	pdf(file=paste0('heatmap_blocked_',CTS_ID,'_scATAC_chIPseq_',key,'_v3_coding_target.pdf'), height=4)  
	print(p)
	dev.off()
...

{
	...
	mat = read.table(paste0('../binary_annot_',CTS_ID,'_scATAC_Maven2023_gene_ISL1_v3.tsv'), sep='\t', header=T)
	dim(mat)  #[1] 37  29
...
res = fill_TF_targeting_predicted_edges(graph_TF_list, linkeage_name = 'CM', graph_list, 
                                            ...
                                            descendent_cluster_id = CM_cluster, TF_symbol=key,
                                            HVG=rownames(sce) )
...
res = fill_TF_targeting_predicted_edges(graph_TF_list, linkeage_name = 'CF', graph_list, 
										sce, celltype_col = celltype_col , CT_cluster_id = CP_cluster ,
                                        descendent_cluster_id = CF_cluster, TF_symbol=key,
                                        HVG=rownames(sce) )
