...
updir = "/Users/felixyu/Documents/GitHub/TIPS/examples/IbarraSoria2018/results/GSE181346_heart_scATAC"
...
key_TFs = lapply(files, function(x) strsplit(x, '_')[[1]][3] %>% unlist) %>% unlist %>% unique
...
# 4       CM  ETV2 SFRP5  0.00000000 -0.0404144174 -0.04041442 0.04041442
# 5       CM  ETV2 SFRP5  0.00000000 -0.0404144174 -0.04041442 0.04041442
# 6       CM GATA4 MYOCD  0.03826759  0.0103243658 -0.02794322 0.02794322
#   direction  status rank                          TF_highConf
# 1  decrease changed    1                                     
# 2  decrease changed    2            CBFB (directAnnotation). 
# 3  increase changed    3            CBFB (directAnnotation). 
...
g_merged <- make_merged_TIPS_graph(subset(final_table, linkeage=='CM'),
	CHD=CHD, 
    added_TF= setdiff(key_TFs, V(graph_list[[paste0('CTS_',CTS_name)]])$name %>% toupper), top_n_label=5,  
    g_string=graph_list[[paste0('CTS_',CTS_name)]])
