...
wd = "/project/xyang2/felixy/2018_weighted/" # Run on HPC
...
file = paste0(inputDir, db, '_STRING_graph_perState_simplified_',s,'weighted.rds')
...
if(V_attack){
	...
	for(j in colnames(attac_V_random)){
		...
		for(i in 1:N){   
		  ...
		  attac_V_random[i,j] = MLmetrics::Area_Under_Curve(res$removed.pct, res$comp.pct)
		 }
	}
...
if(E_attack) {
	...
	# j = 16 
		...
		if( length(E(g0))>1) for(i in 1:N){      
		 ...
		 attac_E_random[i,j] = MLmetrics::Area_Under_Curve(res$removed.pct, res$comp.pct)
		 }
	}
