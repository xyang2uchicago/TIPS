...
################################################################
{
  ...
  df <- readRDS(file = file.path(inputdir, (paste0("df_PAGERANK_strength_ANND.rewiring.P.rds"))))

  df <- df[endsWith(as.character(df$signature), "_cardiac.a"), ]
  ...
  boxplot_strength <- ggplot(df, aes(x = signature, y = normalized.strength, colour = PPI_cat)) +
    ...
    stat_compare_means(
      comparisons = list(
        c("CTS_cardiac.a", "HiGCTS_cardiac.a"),
        c("CTS_cardiac.a", "HiG_cardiac.a"),
        c("HiGCTS_cardiac.a", "HiG_cardiac.a")
      ),
      method = "wilcox.test",
      label = "p.signif",
...
################################################################
{
  ...
  setnames(failure.dt, old = "HiG_blood", new = "signature")

  # attack.edge.btwn = readRDS( file=paste0('attack.edge.btwn.rds'))
  attack.vertex.btwn <- readRDS(file = paste0("attack.vertex.btwn.rds"))

  failure.dt <- failure.dt[grepl("_cardiac.a$", failure.dt$signature), ]
  attack.vertex.btwn <- attack.vertex.btwn[grepl("_cardiac.a$", attack.vertex.btwn$signature), ]

  robustness.dt <- rbind(failure.dt, attack.vertex.btwn[, 1:6]) # , attack.edge.btwn[,1:6])
  (dim(robustness.dt)) #  874   9
...
###################################################
{
  ...
  edge_data <- extract_edge_weights_by_category(graph_list_cardiac, PPI_color_palette, "cardiac.a")

  (head(edge_data, 3))
  #          sample PPI_cat edge_weight num_edges cluster_ID cluster_cat
  # 1 HiG_cardiac.a     HiG        0.01      3072  cardiac.a    unstable
  # 2 HiG_cardiac.a     HiG        0.01      3072  cardiac.a    unstable
  # 3 HiG_cardiac.a     HiG        0.01      3072  cardiac.a    unstable
  ...
  # 1 edge_weight CTS    HiGCTS    17     5       45  0.664 0.664 ns
  # 2 edge_weight CTS    HiG       17  3072    25174. 0.611 0.664 ns
  # 3 edge_weight HiGCTS HiG        5  3072     6970  0.476 0.664 ns
}
...
dir.create("plots_cardiac.a", showWarnings = FALSE)

file_names <- paste0(letters[seq_along(plots)], ".pdf")

for (i in seq_along(plots)) {
  filename <- file.path("plots_cardiac.a", file_names[i])
  plot_obj <- plots[[i]]
