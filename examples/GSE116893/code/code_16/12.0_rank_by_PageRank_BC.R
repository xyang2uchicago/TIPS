...
CP_cluster  <- "16"  # focal cluster; drives HiG_X, CTS_X, HiGCTS_X signatures
...
if (HiGCTS_sig %in% signatures_found) {
    (seed_TF <- subset(res_pr[[HiGCTS_sig]])$gene) # character(0)
    (seed_TF <- intersect(
        seed_TF,
        subset(res_bw[[HiGCTS_sig]], BetweennessCentrality > 0)$gene # character(0)
    ))
} else {
