...
g_degree <- ggplot(data=df, aes(x=k, y=x, col=PPI_cat)) +
           ...
           ylab('average GRN normalized degree') + ggtitle('GSE175634')
  ...
  V_deg_dis$cluster = lapply(V_deg_dis$signature, function(x) unlist(strsplit(x , '_'))[2]) %>% unlist 
    ...
    ## To rovide insights into the distribution of node degrees in each signature and how the degree distribution varies across "transitory" and "steady" PPI_cats,
	...
	g_degree_dis <- ggplot(data= V_deg_dis,
                         aes(x=k, y=degree_distribution, color= cluster, type=PPI_cat )) +  #
		geom_line(aes(linetype=PPI_cat)) + xlab('cumulative degree distribution') +
 ...
 # cumulative NORMALIZED degree distribution to a power law fit ########################
  # PPI_cat: PPI network category: A: CTS; B: CTS&hiG; C; HiG  ############
  #### normalized = FALSE by default
    # V_deg_nor_dis = lapply(graph_list_nonempty, function(x) igraph::degree_distribution(x, cumulative=TRUE, normalized=TRUE )) %>% 
              # lapply(., function(x) x %>% 
                      # as.data.frame(degree_distribution=x)  %>% 
                      # mutate(k=1:length(x))) %>% 
    # rbindlist(.,idcol=names(.))
  ...
  V_deg_nor_dis$cluster = lapply(V_deg_nor_dis$signature, function(x) unlist(strsplit(x , '_'))[2]) %>% unlist 
	...
	g_degree_dis <- V_deg_nor_dis %>%
		  ...
		  geom_line() +
		  xlab('Normalized degree level') + ylab('cumulative normalized  degree distribution') +
	...
	g_degree_dis2 <- V_deg_nor_dis %>%
		  ...
		  scale_color_manual(values = PPI_color_platte) +
		  xlab('Normalized degree level') + ylab('Fraction of nodes having a normalized degree ≥ x') +
			  theme(legend.position = c(0.2, 0.75),
				legend.justification = c(1, 1),
				legend.text = element_text(size = 5)) +
		  coord_trans(x = "log10", y = "log10")
  ...
  g_degree_dis <- ggplot(data=V_deg_dis,
                         aes(x= k, y=degree_distribution, color= cluster, type=PPI_cat )) +  #
   # #                  aes(x=degree_dis, color= PPI_cat, fill=PPI_cat)) +
   # # geom_histogram(aes(y=..density..), position="identity" ,alpha=0.4) +
   # # geom_density(alpha=0.3 ) + xlab('degree distribution') + 
    geom_line(aes(linetype=PPI_cat)) + xlab('degree level (x)') +
    ylab('Fraction of nodes having a degree ≥ x') + 
    theme(legend.position=c(0.2, 0.75), legend.justification=c(1, 1), legend.text = element_text(size=5)) + 
    coord_trans(x="log10", y="log10") #+ #xlab('degree k') +
	#scale_x_reverse()  # Flip the x-axis from highest to smallest  NOT WORK!!! manually flip
 g_degree_dis2 <- ggplot(data=V_deg_dis,
                         aes(x= k, y=degree_distribution, color= PPI_cat )) +  #
    geom_line(aes(group=signature, linetype=PPI_cat)) + xlab('degree level (x)') +
    scale_color_manual(values = PPI_color_platte) +
	ylab('Fraction of nodes having a degree ≥ x') + 
    theme(legend.position=c(0.2, 0.75), legend.justification=c(1, 1), legend.text = element_text(size=5)) + 
    coord_trans(x="log10", y="log10") 
 
 pdf(file='degree_GSE175634.pdf')
  print(g_degree)
  print( g_degree_dis2)
  print(g_degree_dis ) 
  dev.off()  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
# 2:     HiG_0           0.9323077     2     HiG       0     325
# 3:     HiG_0           0.8830769     3     HiG       0     325
...
###################################################
 ## 4.2) evaluate the degree distribution of each PPI_cat; NORMALIZED the cumulative degree is WRONG!!!!!! 
  ...
  # #dev.new(width=14)
  # g_degree_dis <- ggplot(data=V_deg_dis,
                         # aes(x= k, y= normalized_degree_distribution, color= cluster, type=PPI_cat )) +  #
   # # #                  aes(x=degree_dis, color= PPI_cat, fill=PPI_cat)) +
   # # # geom_histogram(aes(y=..density..), position="identity" ,alpha=0.4) +
   # # # geom_density(alpha=0.3 ) + xlab('degree distribution') + 
    # geom_line(aes(linetype=PPI_cat)) + xlab('degree level (x)') +
 ...
 # pdf(file='normalized_degree_GSE175634.pdf')
  # print(g_degree_dis ) 
  # dev.off()  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  dim(V_deg_dis) # [1] 2241   7
  head(V_deg_dis, 3)
   # signature degree_distribution     k PPI_cat cluster n_nodes normalized_degree_distribution
      # <char>               <num> <int>  <fctr>  <char>   <num>                          <num>
# 1:     HiG_0           1.0000000     1     HiG       0     325                     0.00308642
# 2:     HiG_0           1.0000000     2     HiG       0     325                     0.00308642
# 3:     HiG_0           0.9907692     3     HiG       0     325                     0.00305793
	...
	# Boxplot by Categories (NOT USED)
	 ...
	 g2 = ggplot(subset(V_deg_dis, grepl("_CP|_muscle|_endoderm",signature)), 
		  ...
		  stat_compare_means(method = "wilcox", 
                     ...
                     label = "p.signif")
	 g1+g2
	 dev.copy2pdf(file='boxplot_normalized_degree_GSE175634.pdf')	 
