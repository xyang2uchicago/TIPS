...
## Default is TIPS_extend_v2.R (Part 1 dual-pull helpers + Part 2 lineage-lean
## bridge-reclaim, added by Holly 2026-07-29). Override with
## Sys.setenv(TIPS_EXTEND_R = "/path/to/file.R") to pin an older/newer version.
...
tips_source_extend <- function(wd = NULL, code_dir = NULL) {
  ...
  if (!file.exists(path)) {
    stop("TIPS_extend.R not found: ", path, call. = FALSE)
  }
...
tips_configure <- function(
    ...
    wd = Sys.getenv(
      "TIPS_WD",
      "F:/projects/TIPS/source/GSE140802_lineage_tracking/7_data_MuTrans_TIPS_IID/"
    ),
    envir = .GlobalEnv
) {
  ...
  assign(
    "sce_rdata",
    file.path(get("data_dir", envir = envir), "sce_LARRY_mutrans_metacells.RData"),
    envir = envir
  )

  assign("ppi_tag", "IID", envir = envir)
  iid_global_rds <- normalizePath(
    Sys.getenv(
      "TIPS_IID_GLOBAL_RDS",
      "F:/projects/scRNA/results/cardiac_CTS_GRN/GSE175634_iPSC_CM_weighted_IID/iid2025_human_mouse_conserved_global.rds"
    ),
    winslash = "/", mustWork = FALSE
  )
  assign("iid_global_rds", iid_global_rds, envir = envir)
  ...
  assign(
    "graph_notsimp_file",
    file.path(results_dir_val, paste0(db_val, "_IID_graph_perState_notsimplified.rds")),
    envir = envir
  )
  assign(
    "graph_weighted_file",
    file.path(ppi_path_val, paste0(db_val, "_IID_graph_perState_simplified_combinedweighted.rds")),
    envir = envir
  )
  assign(
    "correct_edges_file",
    file.path(results_dir_val, "correct_n_edges_HiG_IID.rds"),
    envir = envir
  )
  assign(
    "correct_edges_review_file",
    sub("\\.rds$", "_auto_pick_review.txt", file.path(results_dir_val, "correct_n_edges_HiG_IID.rds")),
    envir = envir
  )
...
load_larry_sce <- function() {
  .code_dir <- if (exists("code_dir", inherits = TRUE)) {
    ...
    "F:/projects/TIPS/source/GSE140802_lineage_tracking/7_data_MuTrans_TIPS_IID/code_core_4_9vs11"
  }
