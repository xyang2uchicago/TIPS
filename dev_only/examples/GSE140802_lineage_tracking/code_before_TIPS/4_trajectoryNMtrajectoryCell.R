...
step2 = TRUE
rerunslingshot = TRUE
roots = c("Unddiff", "3", "Unddiff_2")
names(roots) = c("Cell.type.clean", "leiden_r0_6", "leiden_r0_6_anno")

if(step2){
  sce   = readRDS("sce.rds")
  range(counts(sce)) #  0.0000 837.5551
  range(logcounts(sce)) #  0.00000 6.73168
  for( r in c("Cell.type.clean", "leiden_r0_6", "leiden_r0_6_anno") )
  { 
    md <- colData(sce) %>%
      as.data.frame() %>%
      mutate(
...
if (step3) {
  ...
  for (r in c("leiden_r0_6", "Cell.type.clean", "leiden_r0_6_anno")) {
...
if(step4){
    ...
    for( r in c("Cell.type.clean", "leiden_r0_6_anno", "leiden_r0_6") ){
