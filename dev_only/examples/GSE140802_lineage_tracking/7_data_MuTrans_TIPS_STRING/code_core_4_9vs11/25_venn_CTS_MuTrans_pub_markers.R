...
intersect(cts_genes, pub_mega)
#[1] "Plscr2" "Tgfbr3"
intersect(TD_CP_CF, pub_mega)
...
TIPS = read.csv(file= file.path(results_dir, "cisTarget_predicted_4/PPI_graph_GRN_prediction_CTS_4_dualpull_final_table.tsv") )
TIPS_A4 = subset(TIPS, linkage='CF')[,c('from','to')] %>% as.data.frame()
TIPS_A4 = c(TIPS_A4[,1], TIPS_A4[,2]) %>% unique
TIPS_A4
#[1] "BHLHE41" "COL1A2"  "GATA1"   "GATA2"   "PCYT1B"  "SPTA1"   "HBB-BS" 

## --- 2×3 Venn grid ---
venn_pdf2 <- file.path(results_dir, "venn_TD_TIPS_HiG.pdf")

pdf(venn_pdf2, width = 12, height = 8)
par(mfrow = c(2, 3), mar = c(3, 3, 2, 1))

## row 1: CTS × HiG × Weinreb
venn(list(TIPS = TIPS_A4, HiG = deg_cm, Baso = pub_baso))
venn(list(TIPS = TIPS_A4, HiG = deg_cf, Mega = pub_mega))
venn(list(TIPS = TIPS_A4, HiG = deg_third, Third = pub_third))


## row 2: CTS × TD × Weinreb
venn(list(TIPS = TIPS_A4, TD = TD_CP_CM, Baso = pub_baso))
venn(list(TIPS = TIPS_A4, TD = TD_CP_CF, Mega = pub_mega))
venn(list(TIPS = TIPS_A4, TD = TD_CP_third, Third = pub_third))

dev.off()
message("[25] wrote ", venn_pdf2)
