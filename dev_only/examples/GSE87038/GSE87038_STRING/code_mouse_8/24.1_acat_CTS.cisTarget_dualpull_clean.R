...
rebuild_mat <- TRUE
source(here::here("examples", "cardiac", "GSE87038", "GSE87038_STRING", "code_mouse_8", "24.0_acat_load_input_clean.R"))

## Absolute path — the relative form broke once 24.0 above changes the working
## directory via setwd() before this line runs.
...
## Set TRUE to select key_TFs automatically (PageRank + cisTarget), ported from PI's
## GSE140802_lineage_tracking/7_data_MuTrans_TIPS_STRING/code_core_4_9vs11/24.1...R.
## Default FALSE preserves the existing manual TCF3/HMGA2/RARB literature-review selection
## below unchanged. Try both and compare — keep whichever is biologically relevant here.
use_automatic_key_TFs <- FALSE
top_TF_rank <- 3
gene_top_n  <- 20

## dorothea_mm not loaded by code/24.0 (human-only); needed for the automatic selector.
## toupper()'d to match this pipeline's uppercase-mouse-as-human-proxy convention.
...
if (length(files) > 0) {
    ...
    ## code_core_4_9vs11/24.1_acat_CTS.cisTarget_dualpull_clean.R.
    ## ====================================================================
    ...
    ppi_path_auto <- paste0(wd, "results_core/PPI_weight/")
    df_PageRank <- readRDS(file.path(ppi_path_auto, "df_PAGERANK_strength_ANND.rewiring.P.rds"))

    tf_mouse_u <- toupper(as.character(TF_mouse))
    ...
    solo_tfs <- intersect(solo_tfs, tf_mouse_u)

    motif_tfs <- split_motif_regulators(motifAnnot_sub$regulators)
    motif_tfs <- intersect(toupper(motif_tfs), tf_mouse_u)
    ...
    if (!length(key_TFs)) {
      stop("No key_TFs from automatic selector — set use_automatic_key_TFs <- FALSE or check NES_threshold", call. = FALSE)
    }
    } else {
    ...
    # Rebuild x fresh, one column per final key_TF (first match) -- the broader
    # intersect() above matches ANY column containing ANY extended-list gene name, so its
    # length is coincidental (happened to equal length(key_TFs) under hg38; mm10's
    # different motif clustering broke that assumption, leaving a stray NA name and
    # crashing heatmap_pull_candidate downstream.
    ...
    x
    } # end of if (use_automatic_key_TFs) ... else ...
...
## (ad-hoc HMGA2-specific check from the original hg38 run; guarded -- under mm10, HMGA2's
## CF subnetwork can come out empty, in which case fill_TF_targeting_predicted_edges()
## returns g_descendant_sub=NULL and there is nothing to verify.)
if ("HMGA2" %in% key_TFs && file.exists(paste0("PPI_graph_HMGA2_GRN_prediction_", CTS_name, "_v3.rds"))) {
    ...
    if (!is.null(res[[1]]) && !is.null(res[[2]])) {
        print(E(res[[2]])$weight)
        print(E(res[[1]])$weight)
        ...
        message("[24.1] HMGA2 CF subnetwork empty under mm10 — skipping shrinkage verification")
    }
}
