...
## Native mouse case kept (not toupper'd): matches TF_mouse (dorothea_mm) and mm10 cisTarget.

res_pr <- rank_TF_CHD_in_PPIN(df_PageRank, CHD, TF_mouse,
...
## Native mouse case kept (not toupper'd) — see note above res_pr.

res_bw <- rank_TF_CHD_in_PPIN(df_betweenness, CHD, TF_mouse,
    signatures = signatures_found,
...
if (HiGCTS_sig %in% signatures_found) {
    (seed_TF <- subset(res_pr[[HiGCTS_sig]])$gene) #   "Isl1" "Irx3" "Alx1"
    (seed_TF <- intersect(
        seed_TF,
